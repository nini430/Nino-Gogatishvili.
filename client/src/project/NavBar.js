        ...
        <ContextConsumer>
            {
                (value)=>(
                  value.categories?.map(item=>(
                   ...
                   <NavLink key={item.name} to={item.name==="all"?"/":item.name}>
...
<path d="M32.0988 29.6014C32.1313 29.9985 31.8211 30.339 31.4268 30.339H1.59438C1.2009 30.339 0.890922 30.0002 0.922082 29.6037L3.06376 2.34717C3.09168 1.9927 3.38426 1.71973 3.73606 1.71973H29.1958C29.5468 1.71973 29.8391 1.99161 29.868 2.34499L32.0988 29.6014Z" fill="url(#paint0_linear_150_361)"/>
                ...
                <ContextConsumer>
                  {
                    value=>(
                      <IconWrapper>
                      <Currency onClick={()=>value.showHideCurrency()}>
                      <span className="currency">{value.currentSymbol}</span>
                      <span  className="downArrow"><svg  width="8" height="4" viewBox="0 0 8 4" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M1 0.5L4 3.5L7 0.5" stroke="black" strokeLinecap="round" strokeLinejoin="round"/>
        </svg></span>
        </Currency>
        <Cart onClick={()=>value.showHideCartOverlay()}>
        <span className="cart">
...
<path d="M15.6875 13.9814C14.4875 13.9814 13.498 14.9277 13.498 16.0752C13.498 17.2226 14.4876 18.1689 15.6875 18.1689C16.8875 18.1689 17.877 17.2226 17.877 16.0752C17.8565 14.9284 16.8875 13.9814 15.6875 13.9814ZM15.6875 16.9011C15.2031 16.9011 14.8239 16.5385 14.8239 16.0752C14.8239 15.612 15.2031 15.2493 15.6875 15.2493C16.172 15.2493 16.5512 15.612 16.5512 16.0752C16.5512 16.5188 16.1506 16.9011 15.6875 16.9011Z" fill="#43464E"/>
        ...
        {value.totalQuantity!==0?<span className="circleCart">{value.totalQuantity}</span>:null }
